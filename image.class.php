...
class Image
{
     ...
     */
    public function thumb($name, $width, $height, $qz = "he110_")
    {
         // 获取图片宽度、高度、及类型信息
     ...
     */
    public function waterMark($groundName, $waterName, $waterPos = 0, $qz = "he110_")
    {
        // 获取水印图片是当前路径，还是指定了路径
        ...
        if (file_exists($curpath . $groundName) && file_exists($wpath . $waterName)) {
            ...
            $groundImg = $this->copyImage($groundImg, $waterImg, $pos, $waterInfo);
             // 通过本类的私有方法，保存加水图片并返回新图片的名称，默认以 "he110_" 为前缀
     ...
     */
    public function cut($name, $x, $y, $width, $height, $qz = "he110_")
    {
        // 获取图片信息
        $imgInfo = $this->getInfo($name);
         // 裁剪的位置不能超出背景图片范围
        ...
        imagedestroy($back);
         // 通过本类的私有方法，保存剪切图并返回新图片的名称，默认以 "he110_" 为前缀
